...
export function parseWhatsAppMessage(messageText: string): ExtractedMessageData {
  ...
  // Extract Applicant Name - handles typos like "Applicat Name"
  ...
  if (applicantMatch && applicantMatch[1]) {
    ...
    if (applicantAltMatch && applicantAltMatch[1]) {
      ...
      // Try typo pattern: "Applicat Name" (missing 'n')
      const typoMatch = cleanText.match(/applicat\s*[:\-]+\s*(.+?)(?:\n|$)/i);
      if (typoMatch && typoMatch[1]) {
        result.applicantName = typoMatch[1].trim();
      }
    }
  }
